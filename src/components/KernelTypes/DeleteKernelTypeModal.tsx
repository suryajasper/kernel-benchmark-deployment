...
interface DeleteKernelTypeModalProps {
  isOpen: boolean;
  onClose: () => void;
  onConfirm: () => void;
  kernelType: KernelTypeDefinition | null;
  isLoading?: boolean;
}

export default function DeleteKernelTypeModal({
  isOpen,
  onClose,
  onConfirm,
  kernelType,
  isLoading = false,
}: DeleteKernelTypeModalProps) {
  if (!kernelType) return null;
      ...
      <ModalBody>
        <div className="space-y-4">
          <p className="text-gray-600">
            Are you sure you want to delete the kernel type{" "}
            <strong>"{kernelType.displayName}"</strong>?
          </p>

          <div className="bg-red-50 border border-red-200 rounded-lg p-4">
            <h4 className="font-medium text-red-800 mb-2">This action will:</h4>
            <ul className="text-sm text-red-700 space-y-1">
              <li>• Permanently remove this kernel type from the database</li>
              <li>
                • Remove all {kernelType.attributes.length} attribute
                definitions
              </li>
              <li>• Potentially affect existing kernels of this type</li>
            </ul>
          </div>
